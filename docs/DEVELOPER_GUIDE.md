# Developer Guide: Sovereign Patterns Bible
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/patterns-bible.git
cd patterns-bible
...
```bash
export SBB_PATTERN_RULES_PATH=your_value_here
export SBB_DEBT_THRESHOLD=your_value_here
export SBB_REPORT_FORMAT=your_value_here
export SBB_AUTO_FIX=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-patterns-bible)
...
```bash
# Build
docker build -t sovereign-patterns-bible:dev .

# Run with env file
docker run --env-file .env -p 8794:8794 sovereign-patterns-bible:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-patterns-bible
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8794`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-patterns-bible",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8794` in use | `lsof -i :8794` | Kill PID or change `SERVICE_PORT` |
